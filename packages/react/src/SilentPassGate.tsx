...
import { useSilentPass, type UseSilentPassOptions } from './useSilentPass.js';

export interface SilentPassGateProps extends UseSilentPassOptions {
 ...
 *
 * ```tsx
 * <SilentPassGate contractAddress={ADDR} connect={connectWallet} threshold={18}>
 *   <SecretBar />
 * </SilentPassGate>
 * ```
 */
export function SilentPassGate(props: SilentPassGateProps) {
  const { children, label, style, unstyled, className, threshold = 18, ...opts } = props;
  const zk = useSilentPass({ ...opts, threshold });
