...
import { EXPLORER, contractUrl, short } from '../lib/format.js';
import { Check, Lock, Upload, Wallet } from './Icons.js';
import { VerificationHistory } from './VerificationHistory.js';
import { PrivacyHealthBadge } from './PrivacyHealthBadge.js';

function Stepper({ ctl }: { ctl: SilentPassController }) {
...
function ContractPanel({ ctl }: { ctl: SilentPassController }) {
  const busy = ctl.busy;
  ...
  return (
    <div className="panel">
      <div className="panel__top">
        ...
        <span className="panel__title">SilentPass contract</span>
...
function CredentialPanel({ ctl }: { ctl: SilentPassController }) {
  ...
  return (
    <div className={`panel ${locked ? 'locked' : ''}`}>
      ...
      <p className="panel__desc">
        The Verification Issuer reads your ID with a vision model, then attests. Only a commitment reaches the chain.
      </p>
...
function ProofPanel({ ctl }: { ctl: SilentPassController }) {
...
function Receipt({ ctl }: { ctl: SilentPassController }) {
...
function ActivityLog({ ctl }: { ctl: SilentPassController }) {
  return (
    <div className="log">
...
export function Console({ ctl }: { ctl: SilentPassController }) {
  return (
    <section className="section section--paper2" id="try">
      <div className="wrap">
        <div className="section__head" data-reveal>
          <p className="kicker">Try it, on {ctl.session?.networkId || 'preview'}</p>
        ...
        <div className="console__bar" data-reveal>
          <div className="console__status">
            <span className={`tag ${ctl.connected ? 'ok' : ''}`}>
              {ctl.connected ? `${ctl.session?.networkId || 'preview'}, connected` : `${ctl.session?.networkId || 'preview'}, not connected`}
            </span>
            <span>{ctl.connected && ctl.session ? `proof server: ${ctl.session.proverServerUri}` : 'proofs run in your wallet'}</span>
          </div>
          {!ctl.connected && (
        ...
        {!ctl.connected ? (
          <div className="connect-card" data-reveal>
            ...
            <p>Use Lace or 1AM with the DApp Connector, set to {ctl.session?.networkId || 'preview'}. Proofs are generated by your wallet's proof server (Lace runs one locally; 1AM uses a hosted one), never by this site. You will need a small DUST balance for fees.</p>
          ...
          <>
            ...
            <VerificationHistory activeTxHash={ctl.gate?.receipt?.txHash} />
            <PrivacyHealthBadge networkId={ctl.session?.networkId} contractActive={!!ctl.api} />
          </>
        )}
      </div>
    </section>
