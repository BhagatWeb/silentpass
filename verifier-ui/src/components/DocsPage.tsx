...
const REPO = 'https://github.com/ogsamrat/SilentPass';
const NPM = 'https://www.npmjs.com/package/SilentPass';
      ...
      <div className="wrap dp-shell">
        ...
        <main className="dp-main">
          <section id="overview" className="dp-sec">
            ...
            <h1 className="dp-h1">SilentPass</h1>
            <p className="dp-lede">
            ...
            <div className="dp-pills">
              <a className="dp-pill" href={NPM} target="_blank" rel="noreferrer">npm i SilentPass</a>
            ...
            <p className="dp-p">
              ...
              SilentPass a composable primitive rather than another siloed identity flow: one
...
npm i SilentPass-react SilentPass`} />
          </section>
          ...
          <section id="verify" className="dp-sec">
            ...
            <CodeWindow
              file="verify.ts"
              lang="ts"
              code={`import { Verifier } from 'silentpass';
...
const CONTRACT =
  '${CONTRACT}';
const venue = Verifier.connect(CONTRACT); // indexer only
...
if (verified) grantAccess();`}
          ...
          <section id="prove" className="dp-sec">
            ...
            <CodeWindow
              file="prove.ts"
              lang="ts"
              code={`import { SilentPassAPI, Issuer, Holder } from 'silentpass';

const api = await SilentPassAPI.join(providers, CONTRACT);

// Enroll once. The secret key never leaves the device.
const guest = new Holder(api);
const enrollment = await holder.enroll();

// The issuer attests after an off-chain check, writing only a commitment.
const authority = new Issuer(api);
...
await holder.proveIdentity('Erika Mustermann', 18, { sessionId });`}
          ...
          <section id="react" className="dp-sec">
            ...
            <p className="dp-p">
              <code>SilentPass-react</code> gives you a drop-in gate. The child renders only on a
              verified result.
            </p>
            <CodeWindow
              file="Gate.tsx"
...
<ProveAgeGate
  ...
  <MembersOnly />              {/* shown only when verified */}
            ...
            <CodeWindow
              file="useSilentPass.tsx"
              lang="tsx"
              code={`import { useSilentPass } from 'silentpass-react';
    ...
    : <button onClick={() => zk.proveAgeOver()} disabled={zk.status === 'proving'}>
        {zk.status === 'proving' ? 'Proving' : 'Prove 18+'}
          ...
          <section id="deployment" className="dp-sec">
            ...
            <p className="dp-p">
              SilentPass runs on Midnight preprod as a single canonical deployment. The demo and every
          ...
          <section id="api" className="dp-sec">
            ...
            <h3 className="dp-h3">SilentPassAPI</h3>
            <ul className="dp-api">
              <li><code>SilentPassAPI.join(providers, address)</code> attach to a contract for proving.</li>
              <li><code>SilentPassAPI.deploy(providers)</code> deploy a new contract (issuer becomes the caller).</li>
            </ul>
      ...
      <footer className="foot">
        <div className="wrap foot__in">
          <span>SilentPass, a reusable zero-knowledge credential primitive on Midnight</span>
