...
export function useSilentPass() {
  ...
  const disconnect = useCallback(() => {
    setSession(null);
    setApi(null);
    append('info', 'Wallet disconnected.');
  }, [append]);
  ...
  const deploy = useCallback(
    () =>
      run('deploy', async () => {
        if (!session) return;
        append('info', 'Deploying your own silentpass contract to preprod (sign in your wallet)…');
...
export type SilentPassController = ReturnType<typeof useSilentPass>;
